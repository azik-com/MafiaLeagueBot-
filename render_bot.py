...
async def do_night(cid, ctx, game):
    ...
    ctx.job_queue.run_once(
        night_done, when=NIGHT_TIME,
        data={"cid": cid}, name=f"night_{cid}"
    )

async def night_done(ctx: ContextTypes.DEFAULT_TYPE):
    cid = ctx.job.data["cid"]
    game = GAMES.get(cid)
    if not game or game.phase != "night":
        return
    await finish_night(cid, ctx, game)
...
async def do_day(cid, ctx, game):
    ...
    ctx.job_queue.run_once(
        day_done, when=DAY_TIME,
        data={"cid": cid}, name=f"day_{cid}"
    )

async def day_done(ctx: ContextTypes.DEFAULT_TYPE):
    cid = ctx.job.data["cid"]
    game = GAMES.get(cid)
    if not game or game.phase != "day":
        return
    await finish_day(cid, ctx, game)
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── KUNDUZ ──
        ...
        if all(p.day_vote is not None for p in game.alive_list()):
            for job in ctx.job_queue.get_jobs_by_name(f"day_{game.chat_id}"):
